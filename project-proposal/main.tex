...
\title{Taste-Based Social Discovery Platform}
...
\author{
  Tulika Jain, CSED%
  \thanks{Roll No: \texttt{1024160110}, %
    Email: \texttt{tjain\_be24\@thapar.edu}}
  \and 
  Mukul, CSED%
  \thanks{Roll No: \texttt{1024160124}, %
    Email: \texttt{mmukul\_be24\@thapar.edu}}
  \and
  Pranav Tiwari, CSED%
  \thanks{Roll No: \texttt{1024160127}, %
    Email: \texttt{ptiwari1\_be24\@thapar.edu}}
}
...
\begin{itemize}[leftmargin=0.7cm]
    \item We will deliver meaningful increments quickly by prototyping the core taste-matching workflow (profile $\rightarrow$ Taste DNA $\rightarrow$ compatibility score) and validating it with real users within a short iteration window.
...
\begin{itemize}[leftmargin=0.7cm]
    \item \textbf{Taste DNA profile:} users build an interest profile from keywords spanning music, movies, and books at launch, with additional categories (memes, games, shows, sports, food, photography, custom/niche tags) added in later iterations.
...
\begin{enumerate}[leftmargin=0.7cm]
    \item User signs up, creates a profile, and selects interest keywords to build their Taste DNA.
...
\begin{itemize}[leftmargin=0.7cm]
    \item Represent each user's Taste DNA as a set/vector of weighted keywords across categories.
...
\BvrHint{\ldots mobile/web-based solution}}
A typical mobile-client / backend-API / database architecture:
\begin{itemize}[leftmargin=0.7cm]
    \item \textbf{Frontend:} Flutter mobile client for profile, explore, matching, chat, and community screens.
    \item \textbf{Backend API:} FastAPI service handling authentication, profile/Taste DNA management, matching computation, connections, and chat metadata.
    \item \textbf{Database:} MongoDB storing user profiles, keywords, content, Drops, connections, communities, and reports.
\end{itemize}
...
\begin{itemize}[leftmargin=0.7cm]
    \item Target: MCR $\ge$ 20\% during the pilot window.
...
\begin{itemize}[leftmargin=0.7cm]
    ...
    \item \textbf{Taste DNA completion rate:} percentage of new users who complete a minimum viable Taste DNA (e.g., $\ge$ 5 keywords across $\ge$ 2 categories) during onboarding.
...
\BvrHint{\ldots first iteration (MVP)}}
\begin{itemize}[leftmargin=0.7cm]
    \item User authentication and profile creation
    \item Taste DNA keyword selection across music, movies, and books
...
stays centered on the core loop: building a Taste DNA, computing an explainable match, and
connecting.
...
\begin{itemize}[leftmargin=0.7cm]
    \item \textbf{Data privacy / consent:} minimize collected personal data; keep interest visibility opt-in; never store plaintext passwords; keep secrets in environment variables, not the frontend.
    \item \textbf{Cold-start matching:} with few pilot users, compatibility scores may be sparse or low-quality; mitigate by seeding a diverse pilot cohort and lowering the minimum-keyword threshold for early matches.
    \item \textbf{Content moderation risk:} user-uploaded profile images and Drops could surface inappropriate content, and this risk grows once memes and chat are added in later iterations; mitigate with upload validation, reporting flows, and admin moderation built in from the first iteration rather than bolted on later.
    \item \textbf{Evaluation measurement ambiguity:} instrument recommendation-impression and connection-request events with clear timestamps before development begins.
    \item \textbf{Operational issues in pilot:} use staging early; ensure CI/CD produces repeatable deployments to reduce pilot downtime.
\end{itemize}

\section{Summary}
This project proposes a deployable taste-based social discovery platform that helps users find and connect with people who share similar interests across multiple content domains, rather than a single category or a follower-based model. It meets the course criteria by emphasizing time-to-value through rapid iterations starting with a lightweight, explainable matching algorithm, implementing CI/CD to support frequent safe releases, and using measurable evaluation criteria (especially match-to-connection rate). It remains focused on engineering workflow, security, and scalability readiness in its MVP, while reserving semantic/embedding-based matching as a well-scoped future enhancement rather than a first-iteration research dependency.
